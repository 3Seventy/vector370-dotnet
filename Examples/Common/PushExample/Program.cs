...
namespace PushExample
{
    class Program
    {
        ...
        static void Main(string[] args)
        {
            ...
            // User name and secret are pulled from App.config by default.
            // (For web applications this will be Web.config)
            var context = new T70Context();

            var actionPushRepo = context.Repository<ActionPushCampaign>(new {AccountId = ACCOUNT_ID});

            PushCampaignExample(actionPushRepo);
        ...
        private static void GetPushActionExample(IRepository<ActionPushCampaign> actionPushRepo)
        {
            var e = actionPushRepo.Get(4251);

            Console.WriteLine("Fetched event: {0}", e.Id);
            Console.WriteLine("Channels: {0}", String.Join(", ", e.Channels));
